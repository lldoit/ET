...
namespace PrimeTween {
    ...
    public
        ...
        public static Sequence Create(int cycles = 1, SequenceCycleMode cycleMode = SequenceCycleMode.Restart, Ease sequenceEase = Ease.Linear, bool useUnscaledTime = false, UpdateType updateType = default) {
            ...
            if (sequenceEase == Ease.Default) { // p2 todo this is questionable because passing Default requires an explicit effort from the user. Replace with PrimeTweenManager.defaultEase?
                sequenceEase = Ease.Linear;
            }
        ...
        /// <param name="warnIfTargetDestroyed">https://github.com/KyryloKuzyk/PrimeTween/discussions/4</param>
        public Sequence GroupCallback([NotNull] Action callback, bool warnIfTargetDestroyed = true) {
            if (TryManipulate()) {
        ...
        /// Can be thought of as an "OnStart" callback of a group.</summary>
        /// <param name="warnIfTargetDestroyed">https://github.com/KyryloKuzyk/PrimeTween/discussions/4</param>
        public Sequence GroupCallback<T>([NotNull] T target, [NotNull] Action<T> callback, bool warnIfTargetDestroyed = true) where T: class {
            if (TryManipulate()) {
        ...
        internal static Tween? CreateCallback<T>([NotNull] T target, Action<T> callback, bool warnIfTargetDestroyed = true) where T: class {
        ...
        public void Complete() {
            if (isAlive && TryManipulate(false)) {
                ...
                root.isPaused = false;
                Assert.IsTrue(root.tween.data.IsMainSequenceRoot());
                root.tween.managedData.UpdateSequence(timeScale > 0f ? float.MaxValue : -1000f, false, true, false, false, ref root.tween.data);
                Assert.IsFalse(isAlive);
            }
        }
        ...
        internal struct SequenceDirectEnumerator {
            ...
            public
                ...
                readonly
                #endif
                ColdData Current { // p2 todo return TweenData?
                get {
                    Assert.IsTrue(sequence.isAlive);
                    Assert.IsNotNull(current);
        ...
        public
        #endif
        Sequence ResetOnComplete() {
            root.ResetOnComplete();
            return this;
        }
