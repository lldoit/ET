...
namespace PrimeTween {
    ...
    internal struct TweenData : IEquatable<TweenData> {
                ...
                if (d.IsDone(cyclesDiff)) { // sequence
                    if (d.resetOnComplete && d.IsMainSequenceRoot()) {
                        ...
                        foreach (var t in GetSequenceSelfChildren(false)) {
                            t.managedData.UpdateSequenceChild(0f, true, invertEase, ref t.data);
                            if (isEarlyExitAfterChildUpdate(ref d)) {
                                break;
                            }
                        }

                        // p1 todo implement resetting correctly. See implementation in TweenAnimation
                        /*bool isBackward = endValue.single == 0f;
                        if (isBackward) {
                            foreach (var child in GetSequenceSelfChildren(true)) {
                                var tween = child.tween;
                                int tweenCyclesTotal = tween.settings.cycles;
                                Assert.AreNotEqual(-1, tweenCyclesTotal); // nested animations can't be infinite
                                tween.cyclesDone = tweenCyclesTotal; // setting the cyclesDone to tweenCyclesTotal prevents callback from firing because isDone requires 'cyclesDiff > 0'
                                tween.UpdateSequenceChild(float.MaxValue, false, false);
                        }
                        } else {
                            foreach (var child in GetSequenceSelfChildren(false)) {
                                var tween = child.tween;
                                tween.SetFlag(TweenData.Flags.StateBefore, false);
                                tween.SetFlag(TweenData.Flags.StateRunning, false);
                                tween.SetFlag(TweenData.Flags.StateAfter, false);
                                tween.UpdateSequenceChild(-1000f, false, false);
                            }
                        }*/
                    }
        ...
        internal void UpdateSequenceChild(float encompassingElapsedTime, bool isRestart, bool invertEase, ref UnmanagedTweenData d) {
            if (d.IsSequenceRoot()) {
        ...
        bool UpdateAndSetElapsedTimeTotal(float newElapsedTimeTotal, out int cyclesDiff, bool invertEase, ref UnmanagedTweenData d) {
        ...
        internal void OnComplete([CanBeNull] Action _onComplete, bool warnIfTargetDestroyed) {
            ...
            cold.data.warnIgnoredOnCompleteIfTargetDestroyed = warnIfTargetDestroyed;
        ...
        void ReportOnComplete(ref UnmanagedTweenData d, bool canResetOnComplete = true) {
            ...
            if (canResetOnComplete && d.resetOnComplete && !d.isInSequence) {
                // reset Tween
                UpdateAndSetElapsedTimeTotal(0f, out _, false, ref d);
            }
        ...
        internal float CalcEasedT(float t, int cyclesDone_, bool invertEase, ref UnmanagedTweenData d) {
            return invertEase ?
                1f - CalcEasedTInternal(1f - t, cyclesDone_, ref d) :
                CalcEasedTInternal(t, cyclesDone_, ref d);
        }
