...
// CODE GENERATOR BEGIN
            ...
            case TweenType.Color: return (target as UnityEngine.SpriteRenderer).color.ToContainer();
            case TweenType.Alpha: return (target as UnityEngine.SpriteRenderer).color.a.ToContainer();
...
// CODE GENERATOR END
    ...
    internal static bool SetAnimatedValue(ref TweenData rt, ref UnmanagedTweenData d) {
        ...
        switch (tweenType) {
            ...
            case TweenType.TweenTimeScale:
            case TweenType.TweenTimeScaleSequence: {
                var tweenTarget = rt.target as ColdData;
            ...
            case TweenType.MaterialColorProperty: (target as UnityEngine.Material).SetColor(rt.intParam, ColorVal()); break;
            case TweenType.MaterialProperty: (target as UnityEngine.Material).SetFloat(rt.intParam, FloatVal()); break;
            case TweenType.MaterialAlphaProperty: {
                var mat = rt.target as UnityEngine.Material;
                var _propId = rt.intParam;
                mat.SetColor(_propId, mat.GetColor(_propId).WithAlpha(FloatVal()));
                break;
            }
            case TweenType.MaterialTextureOffset:   (target as UnityEngine.Material).SetTextureOffset(rt.intParam, Vector2Val()); break;
            case TweenType.MaterialTextureScale:    (target as UnityEngine.Material).SetTextureScale (rt.intParam, Vector2Val()); break;
            case TweenType.MaterialPropertyVector4: (target as UnityEngine.Material).SetVector       (rt.intParam, Vector4Val()); break;

            case TweenType.ShakeLocalRotation: (rt.target as UnityEngine.Transform).localRotation = d.startValue.quaternion * UnityEngine.Quaternion.Euler(Tween.getShakeVal(ref rt, ref d)); break;
            case TweenType.ShakeScale:         (rt.target as UnityEngine.Transform).localScale =    d.startValue.vector3 + Tween.getShakeVal(ref rt, ref d); break;
            case TweenType.ShakeLocalPosition: (rt.target as UnityEngine.Transform).localPosition = d.startValue.vector3 + Tween.getShakeVal(ref rt, ref d); break;
            ...
            case TweenType.Color: (target as UnityEngine.SpriteRenderer).color = ColorVal(); break;
            ...
            #endif
            case TweenType.MaterialColor: (target as UnityEngine.Material).color = ColorVal(); break;
            case TweenType.MaterialAlpha: {
                var _target = target as UnityEngine.Material;
                var val = FloatVal();
                _target.color = _target.color.WithAlpha(val);
                break;
            }
            case TweenType.MaterialMainTextureOffset: (target as UnityEngine.Material).mainTextureOffset = Vector2Val(); break;
            case TweenType.MaterialMainTextureScale: (target as UnityEngine.Material).mainTextureScale = Vector2Val(); break;
            #if !UNITY_2019_1_OR_NEWER || AUDIO_MODULE_INSTALLED
    ...
    internal static (PropType, Type) TweenTypeToTweenData(TweenType tweenType) {
        switch (tweenType) {
            ...
            case TweenType.Color: return (PropType.Color, typeof(UnityEngine.SpriteRenderer));
            case TweenType.Alpha: return (PropType.Float, typeof(UnityEngine.SpriteRenderer));
