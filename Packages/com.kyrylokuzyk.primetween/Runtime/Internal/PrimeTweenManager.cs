    ...
    internal sealed class PrimeTweenManager : MonoBehaviour {
        ...
        static GameObject CreateNewGameObject() {
            ...
            if (Application.isEditor) {
                ...
                if (!EditorApplication.isPlaying && _playModeState != PlayModeStateChange.EnteredEditMode) {
        ...
        internal static bool HasCurrentTweenAnimations => HasInstance && Instance.currentTweenAnimations.Count != 0;
        ...
        static void IniOnLoad() {
            ...
            EditorApplication.playModeStateChanged += state => {
                ...
                DebugLifetimeStatic($"playModeStateChanged: {state}");
                switch (state) {
            ...
            return;
            #endif
            Debug.Log("All tweens were stopped because of 'Recompile And Continue Playing'.");
        ...
        internal void UpdateTweens(_UpdateType updateType, float? deltaTime = null, float? unscaledDeltaTime = null) {
            switch (updateType) {
                ...
                case _UpdateType.Update:
                    ...
                    UpdateTweenArray(tweensUpdate, deltaTime ?? Time.deltaTime, unscaledDeltaTime ?? Time.unscaledDeltaTime);
                    break;
                case _UpdateType.LateUpdate:
                    ...
                    UpdateTweenArray(tweensLateUpdate, deltaTime ?? Time.deltaTime, unscaledDeltaTime ?? Time.unscaledDeltaTime);
                    AddNewTweens(updateType);
                    break;
                case _UpdateType.FixedUpdate:
                    AddNewTweens(updateType);
                    UpdateTweenArray(tweensFixedUpdate, deltaTime ?? Time.fixedDeltaTime, unscaledDeltaTime ?? Time.fixedUnscaledDeltaTime);
                    break;
                default: throw new Exception($"Invalid update type: {updateType}");
            }
        }
        ...
        internal void warnStructBoxingInCoroutineOnce(long id, UnityEngine.Object target) {
